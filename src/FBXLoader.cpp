...
{
	level = 0;
	//Initialise the SDK manager, handles memory management
	...
	//Call the initialise method which will lod the contents of the FBX File
...
void processNode(FbxNode *node, MeshData *meshData)
{
	...
	cout << "Node " << nodeName << "Position " << translation[0] << " " << translation[1] << " " <<
		"Rotation " << rotation[0] << " " << rotation[1] << " " <<
		"Scale " << scaling[0] << " " << scaling[1] << " " << endl;
	...
	for (int iPolygon = 0; iPolygon < mesh->GetPolygonCount(); iPolygon++)
	{
		for (unsigned iPolygonVertex = 0; iPolygonVertex < 3; iPolygonVertex++)
		{
			...
			if (fbxLayerUV)
			{
				...
				verts[fbxCornerIndex].texCoords.y = fbxUV[1];
			}
		}
	}
}